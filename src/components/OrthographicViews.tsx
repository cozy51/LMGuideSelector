...
export function OrthographicViews({conditions,result}:{conditions:Conditions;result:CalculationResult}){
 ...
 // 重心・駆動のラベルと数値は、互いに近づいたときに文字が重ならないよう、
 // 上側にある点は上方向、下側にある点は下方向にまとめて配置する（数値がマーカーに近い側）。
 const cogAbove=frontCog.y<=frontDrive.y;
 const stackText=(y:number,above:boolean)=>({valueY:clamp(above?y-9:y+9,18,206),labelY:clamp(above?y-26:y+26,18,206)});
 const frontCogStack=stackText(frontCog.y,cogAbove);
 const frontDriveStack=stackText(frontDrive.y,!cogAbove);
 const sideCogStack=stackText(sideCog.y,cogAbove);
 const sideDriveStack=stackText(sideDrive.y,!cogAbove);
 const frontCogValue={x:clamp(frontCog.x+14,35,238),y:frontCogStack.valueY};
 const frontDriveValue={x:clamp(frontDrive.x+12,35,228),y:frontDriveStack.valueY};
 const sideCogValue={x:clamp(sideCog.x+14,35,238),y:sideCogStack.valueY};
 const sideDriveValue={x:clamp(sideDrive.x+12,35,218),y:sideDriveStack.valueY};
 return <div className="orthographic-section">
  ...
  <div className="orthographic-grid">
   <figure className="bs-view"><figcaption><div><b>X–Y</b><em>BS = 同一レール上の前後ブロック間隔</em></div><span>長手位置と高さを確認</span></figcaption><svg viewBox="0 0 320 230" role="img" aria-label={`側面図。BSブロックスパン ${layout.blockSpanMm} mm、重心 Xg ${result.cogMm.x} mm、Yg ${result.cogMm.y} mm。駆動X ${drive.positionMm.x} mm、Br ${drive.positionMm.y} mm`}>
    ...
    <g className="ortho-cog" transform={`translate(${sideCog.x},${sideCog.y})`}><circle r="10"/><path d="M-7 0h14M0-7v14"/></g><text className="point-label cog-label" x={sideCogValue.x} y={sideCogStack.labelY}>重心</text><text className="point-value" x={sideCogValue.x} y={sideCogValue.y}>Xg {fmt(result.cogMm.x,0)} / Yg {fmt(result.cogMm.y,0)}</text>
    <g className="ortho-drive" transform={`translate(${sideDrive.x},${sideDrive.y})`}><circle r="8"/><circle r="3"/></g><text className="point-label drive-label" x={sideDriveValue.x} y={sideDriveStack.labelY}>駆動</text><text className="point-value drive-value" x={sideDriveValue.x} y={sideDriveValue.y}>X {fmt(drive.positionMm.x,0)} / Br {fmt(drive.positionMm.y,0)}</text>
   ...
   <figure className="rs-view"><figcaption><div><b>Z–Y</b><em>RS = 左右レールの間隔</em></div><span>横偏心と高さを確認</span></figcaption><svg viewBox="0 0 320 230" role="img" aria-label={`正面図。RSレールスパン ${layout.railSpanMm} mm、重心 Zg ${result.cogMm.z} mm、Yg ${result.cogMm.y} mm。駆動 Bt ${drive.positionMm.z} mm、Br ${drive.positionMm.y} mm`}>
    ...
    <g className="ortho-cog" transform={`translate(${frontCog.x},${frontCog.y})`}><circle r="10"/><path d="M-7 0h14M0-7v14"/></g><text className="point-label cog-label" x={frontCogValue.x} y={frontCogStack.labelY}>重心</text><text className="point-value" x={frontCogValue.x} y={frontCogValue.y}>Zg {fmt(result.cogMm.z,0)} / Yg {fmt(result.cogMm.y,0)}</text>
    <g className="ortho-drive" transform={`translate(${frontDrive.x},${frontDrive.y})`}><circle r="8"/><circle r="3"/></g><text className="point-label drive-label" x={frontDriveValue.x} y={frontDriveStack.labelY}>駆動</text><text className="point-value drive-value" x={frontDriveValue.x} y={frontDriveValue.y}>Bt {fmt(drive.positionMm.z,0)} / Br {fmt(drive.positionMm.y,0)}</text>
